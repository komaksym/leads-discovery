...
Plaintext checkpoints, usage ledgers, provider payloads, credentials, temporary files, debug state, and canary coverage evidence remain runner-private and never enter `generated-leads` or Actions artifacts. Before the first canary-private paid barrier, the canary mirrors only the completed normal restart authority required by the fixed canary: `checkpoint.json`, `usage_events.jsonl`, `companies_evaluated.jsonl`, `contact_checkpoint.json`, `contact_usage_events.jsonl`, `contacts.jsonl`, and `leads.csv`. That bounded snapshot and the exact private canary checkpoint/usage authority are independently authenticated and encrypted into commit metadata on the dedicated `canary-operation-journal` branch. A fresh runner restores the normal snapshot before deciding whether normal `run`/`enrich` can execute, while private pending Clay/Instantly identity is restored before any replacement dispatch can be considered. Missing, conflicting, malformed, or undecryptable durable authority fails closed.

The journal branch tree remains operationally empty. Journal metadata uses only opaque operation hashes/coarse states plus ciphertext; plaintext prospect/contact data is never stored in the journal tree or metadata. The journal is not a publication surface. The encryption key is supplied separately as the `CANARY_STATE_KEY` production-canary Environment secret.
