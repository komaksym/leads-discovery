# M4 Contact Discovery & Enrichment Implementation Plan (completed)

> This plan records the completed implementation. The permanent contract and red-team tests now
> live in `tests/test_m4_*.py`; the remaining external gate is the credentialed one-company canary.
...
- Independent M4 contract-test and red-team evidence has been integrated into `tests/`.
...
- M4 is marked complete in `PLANS.md` after the combined validation and red-team review.
...
- [x] **Step 1: Add `ContactRecord` as a separate persisted model**
...
- [x] **Step 2: Parse structured Exa person entities conservatively**
...
- [x] **Step 3: Rank by decision proximity**
...
- [x] **Step 4: Deduplicate exactly and cap the retained contacts**
...
- [x] **Step 5: Run narrow production checks**
...
- [x] **Step 1: Add a sanitized contact-provider error/result contract**
...
- [x] **Step 2: Implement Exa People Search**
...
- [x] **Step 3: Implement Clay routine start and status read**
...
- [x] **Step 4: Implement Apollo synchronous fallback**
...
- [x] **Step 5: Implement Instantly verification-only methods**
...
- [x] **Step 6: Run narrow production checks**
...
- [x] **Step 1: Validate M4 paths and load accepted M3 companies**
...
- [x] **Step 2: Add a dedicated contact checkpoint and usage replay**
...
- [x] **Step 3: Discover and publish contacts company-by-company**
...
- [x] **Step 4: Run/resume one bounded Clay batch**
...
- [x] **Step 5: Apply Apollo fallback one contact at a time**
...
- [x] **Step 6: Create or resume Instantly verification**
...
- [x] **Step 7: Publish deterministic artifacts after every known progress point**
...
- [x] **Step 8: Return useful partial summaries**
...
- [x] **Step 9: Run narrow production checks**
...
- Modify: `PLANS.md` to record the completed M4 milestone and integrated evidence
...
- [x] **Step 1: Add the `enrich` parser and dry path**
...
- [x] **Step 2: Compose live enrichment only behind `--execute-live`**
...
- [x] **Step 3: Add M4 setup and artifact documentation**
...
- [x] **Step 4: Add a manual-only lead-generation workflow**

Use only `workflow_dispatch`; no schedule. Install the package, invoke `enrich --execute-live`
with secrets, and publish only `leads.csv` and `contacts.jsonl` to the dedicated
`generated-leads` branch. Keep the other run files private to the ephemeral runner. Do not create
outreach or CRM steps.

- [x] **Step 5: Record the completed M4 milestone**

Record M4 scope and contract links in `PLANS.md`, and point to the permanent integrated contract
and red-team tests. Leave the credentialed one-company canary as the external acceptance gate.

- [x] **Step 6: Run full production-branch gates**
...
- [ ] **Step 7: Publish the draft PR (operator handoff)**
...
No live provider smoke test is part of this local implementation; the manual workflow is the
operator-controlled external acceptance step. Publishing a PR is outside this local task.
