...
**Status:** production implementation contract; completion remains gated on combined tests and red-team review.
...
repository secrets and uploads only the five M4 run artifacts with short retention. It has no
schedule, no outreach step, and no write-back to a CRM.

## Completion gate

M4 is not complete on the production branch. Completion requires the production and independent
contract-test candidates to be combined, all lint/type/test/build gates to pass, and the
post-integration red team to have no open confirmed defect. Only then may `PLANS.md` mark M4
complete.